    ...
    <>
      ...
      <div id="wrapper" className="mx-[200px] mb-[80px]">
        ...
        <p className="text-gray-600 prompt">
          ...
          <br></br>
          <br></br>
          Thereafter, for each image in the given train set,
          I resized the image into 256 x 256 shape using cv2.resize() function.

          ( cv2 is a library in python that works as an API for open-cv library. )
          I used the corrseponding point pairs to create a 256 x 256 mask of the input 256 x 256 image using cv2.Polyfill() function.
          This process was repeated for all images present in the training dataset. Label set Y also had binary mask of same size.
          Using binary crossentropy as loss, I trained the U-Net model.  The loss was reduced to the order of 10<sup>-5</sup> which was actually good. (There is a huge chance that the model has overfit to the train set, but actually, it didn't. I tried testing it with some random pics and it worked out pretty well. )
        </p>
      </div>
      <Footer></Footer>
    </>
